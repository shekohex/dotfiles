...
in
{
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      set -g status-right "#[fg=#{@thm_crust},bg=#{@thm_surface_0}] #{?pane_in_mode,COPY,#{prefix_highlight}} #[default]#[fg=#{@thm_crust},bg=#{@thm_rosewater}]#(if [ -n \"$SSH_CONNECTION\" ]; then printf ' %s ' \"$(hostname -s)\"; fi)#[default]#[fg=#{@thm_crust},bg=#{@thm_surface_1}] #{b:pane_current_path} #[default]#[fg=#{@thm_crust},bg=#{@thm_teal}] #{continuum_status} #[default]#[fg=#{@thm_crust},bg=#{@thm_mauve}]#(branch=$(git -C \"#{pane_current_path}\" branch --show-current 2>/dev/null || git -C \"#{pane_current_path}\" rev-parse --short HEAD 2>/dev/null); [ -n \"$branch\" ] && printf ' %s ' \"$branch\")#[default]"
